...
exports.isAdminMutation = isAdminMutation;
exports.safeAdminValues = safeAdminValues;
exports.registerAdminAudit = registerAdminAudit;
...
function isAdminMutation(method, route) {
...
function safeAdminValues(value) {
    ...
    for (const [key, entry] of Object.entries(value)) {
        ...
        if (entry && typeof entry === 'object') {
            ...
            result[key] = entry.slice(0, 1000);
        else if (typeof entry === 'number' || typeof entry === 'boolean' || entry === null)
            result[key] = entry;
    }
    return result;
}
function registerAdminAudit(app) {
    app.addHook('onSend', async (request, reply, payload) => {
        const route = request.routeOptions.url ?? '';
        if (!isAdminMutation(request.method, route))
            return payload;
        ...
        (0, persistent_audit_1.recordAudit)({ category: 'ADMIN', actor, action: 'alteracao_administrativa', step: 'administracao',
            metadata: {
