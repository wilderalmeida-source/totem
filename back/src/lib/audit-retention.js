...
exports.startAuditRetention = startAuditRetention;
...
function startAuditRetention() {
    ...
    const clean = async () => {
        ...
        try {
            ...
            const removed = await prismalog_1.PrismaLog.$executeRaw `
      ...
      `;
            if (removed > 0)
                (0, persistent_audit_1.recordAudit)({ category: 'ADMIN', actor: 'sistema', action: 'logs_retencao_aplicada',
                    step: 'administracao', metadata: { days, cutoff: cutoff.toISOString(), removed, outcome: 'CONCLUIDO' } });
        }
        catch {
            console.error('AUDIT_RETENTION_FAILED');
            (0, persistent_audit_1.recordAudit)({ category: 'ADMIN', actor: 'sistema', action: 'logs_retencao_falhou',
