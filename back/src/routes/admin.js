...
async function adminRoutes(fastify) {
    ...
    fastify.post('/clinux/admin/users', async (request, reply) => {
        ...
        const user = await prismalog_1.PrismaLog.adminUser.create({ data: { username: body.username, displayName: body.displayName, passwordHash: hashPassword(body.password) } });
    ...
    fastify.patch('/clinux/admin/users/:id', async (request) => {
        ...
        const body = zod_1.z.object({ displayName: zod_1.z.string().trim().min(2).max(150).optional(), password: zod_1.z.string().min(10).max(256).optional(), active: zod_1.z.boolean().optional() }).parse(request.body);
        return prismalog_1.PrismaLog.adminUser.update({ where: { id }, data: { displayName: body.displayName, active: body.active, ...(body.password ? { passwordHash: hashPassword(body.password) } : {}) }, select: { id: true, username: true, displayName: true, active: true } });
    });
