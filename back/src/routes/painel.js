...
async function descobrirPainelId(fastify, ipDestino) {
    try {
        ...
        const painel = paineis.find((p) => p.ativo &&
        ...
        sock.on("data", (buf) => {
            ...
            (async () => {
                ...
                try {
                    ...
                    const body = res.json();
                    let dsPaciente;
                    if (Array.isArray(body.senhasRawQuery) &&
                        body.senhasRawQuery.length > 0) {
                        const pacienteRaw = body.senhasRawQuery.find((r) => r.ds_paciente);
                    ...
                    const nomeAux = key.match(/^(?:[^-]*-){5}\s*([^-]*?)\s*-/)?.[1]?.trim() ?? "";
                    const guicheAux = key.match(/^(?:[^-]*-){3}\s*([^-]*?)\s*-/)?.[1]?.trim() ?? "";
                    const text = () => {
                        switch (guicheAux) {
                            case "00":
