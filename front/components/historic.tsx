...
export default function Historic() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Atualização não-bloqueante da UI
  ...
  // Throttle simples para não refazer fetch a cada mensagem
  ...
  useEffect(() => {
    ...
    // 2) Reagir ao WebSocket (aceita "db" ou "tcp" se seu backend variar)
    ...
    }catch{console.log("Erro conexão WS")}};
  ...
  const senhasFiltered = useMemo(() => {
    ...
    // filtra RESULTADO e registros sem atendimentos
  ...
  const entregues = useMemo(
    () => senhasnrUnique.filter((s) => s.dt_saida).length,
    [senhasnrUnique]
  );

  /** ===== Tabelas (memo) ===== **/
  const tabela1: ReactElement[] = useMemo(() => {
    return senhasUnique.map((s) => {
      const pac = s.atendimentos?.[0].pacientes_atendimentos_cd_pacienteTopacientes;
      const key = `a-${pac?.cd_paciente}-${s.dt_saida ? '1' : '0'}`; // chave estável
      return (
        pac?
        <tr
          key={key}
  ...
  const tabela2: ReactElement[] = useMemo(() => {
    return senhasnrUnique.map((s) => {
      const key = `e-${s.ds_paciente}-${s.dt_saida ? '1' : '0'}`; // chave estável
      return (
        <tr
          key={key}
          className="border-b bg-gray-200/[0.6] dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
  ...
  return (
    <>
      <div className="ml-3 flex">
        ...
        <div className="mr-5">
          <div className="ml-3">
            <h1 className="text-xl">Atendimento</h1>
            <h2>
              Total: {tabela1.length} | Atendidos: {atendidos} | Restantes:{' '}
              {tabela1.length - atendidos}
            </h2>
          </div>
            ...
            {!loading && (
              <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 h-4">
                <thead className="text-xs text-gray-700 uppercase bg-gray-300 dark:bg-gray-700 dark:text-gray-400 sticky top-0 bg-primary">
                  <tr>
                    <th scope="col" className="px-6 py-3 w-64 bg-slate-300">
                      Nome
                    </th>
                    <th scope="col" className="px-6 py-3 w-1/5 bg-slate-300">
                      Status
                    </th>
                  </tr>
                </thead>
      ...
      {/* opcional: indicador de atualização em background via transition */}
      {isPending && (
        <div className="ml-3 mt-2 text-xs text-gray-400">Sincronizando dados…</div>
      )}
    </>
  );
}
