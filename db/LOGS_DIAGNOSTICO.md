...
Os eventos usam a tabela `AuditLog` existente no banco de logs. Esta ampliacao nao exige nova tabela.

- No painel **Logs**, pesquise o ID completo do fluxo ou equipamento e abra **Ver detalhes**. A consulta mostra ate 200 registros por filtro, dos mais recentes para os mais antigos.
...
- A auditoria e assincrona e nao impede a operacao. Falhas de rede, encerramento do processo ou saturacao podem perder eventos. As escritas pendentes no backend sao limitadas; `droppedEvents` informa descartes do registrador de fluxo no proximo evento aceito. Falhas de persistencia tambem aparecem no console.
